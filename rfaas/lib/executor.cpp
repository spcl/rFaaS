...
namespace rfaas {
  ...
  rdmalib::Buffer<char, libfabric> libfabric_executor::load_library(std::string path)
  {
    ...
    rewind(file);
    rdmalib::Buffer<char, libfabric> functions(len);
  ...
  rdmalib::Buffer<char, ibverbs> verbs_executor::load_library(std::string path)
  {
    _func_names.clear();
    // Load the shared library with functions code
    FILE* file = fopen(path.c_str(), "rb");
    ...
    size_t len = ftell(file);
    rewind(file);
    rdmalib::Buffer<char, ibverbs> functions(len);
    rdmalib::impl::expect_true(fread(functions.data(), 1, len, file) == len);
  ...
  void libfabric_executor::deallocate()
  {
    if(_exec_manager) {
      _end_requested = true;
  ...
  void verbs_executor::deallocate()
  {
    if(_exec_manager) {
      _end_requested = true;
      // The background thread could be nullptr if we failed in the allocation process
  ...
  void libfabric_executor::poll_queue()
  {
  ...
  void verbs_executor::poll_queue()
  {
    // FIXME: hide the details in rdmalib
    spdlog::info("Background thread starts waiting for events");
  ...
  bool libfabric_executor::allocate(std::string functions_path, int numcores, int max_input_size,
      int hot_timeout, bool skip_manager, rdmalib::Benchmarker<5> * benchmarker)
  {
    rdmalib::Buffer<char, libfabric> functions = load_library(functions_path);
    if(!skip_manager) {
      ...
      _exec_manager.reset(
        new manager_connection<Library>(
    ...
    uint32_t obj_size = sizeof(rdmalib::BufferInformation<libfabric>);
  ...
  bool verbs_executor::allocate(std::string functions_path, int numcores, int max_input_size,
      int hot_timeout, bool skip_manager, rdmalib::Benchmarker<5> * benchmarker)
  {
    rdmalib::Buffer<char, ibverbs> functions = load_library(functions_path);
    if(!skip_manager) {
      // FIXME: handle more than one manager
      servers & instance = servers::instance();
      auto selected_servers = instance.select(numcores);
    ...
    SPDLOG_DEBUG("Allocating {} threads on a remote executor", numcores);
    // Now receive the connections from executors
    uint32_t obj_size = sizeof(rdmalib::BufferInformation<ibverbs>);
