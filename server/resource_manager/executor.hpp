...
namespace rfaas { namespace resource_manager {
  ...
  struct Lease
  {
    ...
    Lease(int cores, int64_t memory, bool total, std::weak_ptr<Executor> && node):
      cores(cores),
      memory(memory),
      total(total),
      node(node)
  ...
  struct Executor : server_data
  {
    ...
    Executor();
    Executor(const std::string & node_name, const std::string & ip, int32_t port, int16_t cores, int64_t memory);

    void initialize_data(const std::string & node_name, const std::string & ip, int32_t port, int16_t cores, int64_t memory);
    void initialize_connection(ibv_pd* pd, rdmalib::Connection* conn);
    bool is_initialized() const;

    bool lease(int cores, int64_t memory);
  ...
  struct Executors
  {
    ...
    std::tuple<std::weak_ptr<Executor>, bool> add_executor(const std::string& name, const std::string & ip, int32_t port, int16_t cores, int64_t memory);
