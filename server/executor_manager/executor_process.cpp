...
ProcessExecutor::ProcessExecutor(int cores, ProcessExecutor::time_t alloc_begin,
                                 pid_t pid)
    : ActiveExecutor(cores), _pid(pid) {
...
int ProcessExecutor::id() const { return static_cast<int>(_pid); }

ProcessExecutor *ProcessExecutor::spawn(const rfaas::AllocationRequest &request,
                                        const ExecutorSettings &exec,
                                        const executor::ManagerConnection &conn,
                                        const Lease &lease) {
  static int counter = 0;
  auto begin = std::chrono::high_resolution_clock::now();
  // spdlog::info("Child fork begins work on PID {} req {}", mypid,
  // fmt::ptr(&request));
  std::string client_addr{request.listen_address};
  std::string client_port = std::to_string(request.listen_port);
  // spdlog::error("Child fork begins work on PID {} req {}", mypid,
  // fmt::ptr(&request));
...
#ifdef USE_GNI_AUTH
  std::string authentication_cookie = std::to_string(
      rdmalib::Configuration::get_instance().credential().value());
#endif
  ...
  if (mypid == 0) {
    ...
    spdlog::info("Child fork begins work on PID {}, using sandbox {}", mypid,
                 sandbox_serialize(sandbox_type));
    ...
    if (sandbox_type == SandboxType::PROCESS) {
      argv = {"executor",
              ...
              "thread",
              "-r",
              executor_repetitions.c_str(),
              "-x",
              executor_recv_buf.c_str(),
              "-s",
              client_in_size.c_str(),
              "--pin-threads",
              executor_pin_threads.c_str(),
              "--fast",
              client_cores.c_str(),
              "--warmup-iters",
              executor_warmups.c_str(),
              "--max-inline-data",
              executor_max_inline.c_str(),
              "--func-size",
              client_func_size.c_str(),
              "--timeout",
              client_timeout.c_str(),
              "--mgr-address",
              conn.addr.c_str(),
              "--mgr-port",
              mgr_port.c_str(),
      ...
      //   nullptr
      // };
      argv = {"docker_rdma_sriov", "run", "--rm", "--net=mynet", "-i", //"-it",
              // FIXME: make configurable
              "--ip=148.187.105.250",
              // FIXME: make configurable
              "--volume", "/users/mcopik/projects/rdma/repo/build_repo2:/opt",
              // FIXME: make configurable
              "rdma-test", "/opt/bin/executor", "-a", client_addr.c_str(), "-p",
              client_port.c_str(), "--polling-mgr", "thread", "-r",
              executor_repetitions.c_str(), "-x", executor_recv_buf.c_str(),
              "-s", client_in_size.c_str(), "--pin-threads",
              executor_pin_threads.c_str(), "--fast", client_cores.c_str(),
              "--warmup-iters", executor_warmups.c_str(), "--max-inline-data",
              executor_max_inline.c_str(), "--func-size",
              client_func_size.c_str(), "--timeout", client_timeout.c_str(),
              "--mgr-address", conn.addr.c_str(), "--mgr-port",
              mgr_port.c_str(), "--mgr-secret", mgr_secret.c_str(),
              "--mgr-buf-addr", mgr_buf_addr.c_str(), "--mgr-buf-rkey",
              mgr_buf_rkey.c_str(), nullptr};
    }

    std::vector<const char *> cstrings_argv;
    std::transform(
        argv.begin(), argv.end(), std::back_inserter(cstrings_argv),
        [](const std::string &input) -> const char * { return input.c_str(); });
    std::copy(additional_args.begin(), additional_args.end(),
              std::back_inserter(cstrings_argv));
    ...
    int ret = execvp(cstrings_argv.data()[0],
                     const_cast<char **>(&cstrings_argv.data()[0]));
    if (ret == -1) {
      spdlog::error("Executor process failed {}, reason {}", errno,
                    strerror(errno));
      close(fd);
      exit(1);
    }
