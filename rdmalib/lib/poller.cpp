...
std::tuple<ibv_wc *, int> Poller::poll(bool blocking, int count) {
  ...
  do {
    ret = ibv_poll_cq(_recv_cq, count == -1 ? _wc_size : count, wcs);
  } while (blocking && ret == 0);

  if (ret < 0) {
    spdlog::error(
        "Failure of polling events from: {} queue! Return value {}, errno {}",
        "recv", ret, errno);
    ...
    for (int i = 0; i < ret; ++i) {
      if (wcs[i].status != IBV_WC_SUCCESS) {
        spdlog::error(
            ...
            "recv", i + 1, ret, wcs[i].status,
            ibv_wc_status_str(wcs[i].status));
      }
      SPDLOG_DEBUG("Queue {} Ret {}/{} WC {} Status {}", "recv", i + 1, ret,
                   wcs[i].wr_id, ibv_wc_status_str(wcs[i].status));
    }
  return std::make_tuple(wcs, ret);
}
#else
std::tuple<fi_cq_data_entry *, int> Poller::poll(bool blocking, int count,
                                                 bool update) {
  int ret = 0;
